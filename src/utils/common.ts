...
export const getStartTime = (timeline: any[], initialValue: string) => {
    ...
    timeline?.map((t: any) => {
        if (t.__typename === 'ReadyForReviewEvent' || (t.__typename === 'UnlabeledEvent' && t.label.name === 'WIP')) {
...
export const isComment = (timelineEvent: any) => {
    ...
    if (timelineEvent.__typename === 'IssueComment' && !bots.includes(timelineEvent.author.login) && timelineEvent.bodyText.length) {
        return true;
    }

    if (timelineEvent.__typename === 'PullRequestReview' && !bots.includes(timelineEvent.author.login) && timelineEvent.bodyText.length) {
        return true;
    }
