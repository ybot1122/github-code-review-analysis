...
const GroupIntervalTextArea = ({arr}: {arr: number[]}) => {
    ...
    arr.map((num) => {
        const key = Object.keys(result).find((interval_start) => num >= Number(interval_start) && num < Number(interval_start) + interval);
    ...
    for (let i = 0; i < keys.length - 1; i++) {
        prettyKeys[i] = ((convertToHrs) ? Math.round(msToHrs(keys[i])) : Math.round(keys[i])) + ' to ' + ((convertToHrs) ? Math.round(msToHrs(keys[i+1])) - 1 : Math.round(keys[i+1]) - 1)
    }
    ...
    return <>
        ...
        Set Max Limit<input type="number" onBlur={((e) => setMaxLimit(Number(e.target.value)))} min={min} max={max} /><br />
...
export {
    GroupIntervalTextArea as GroupIntervalButton
}