...
function App() {
  ...
  const [activeArray, setActiveArray] = useState<{arr: any[], name: string}>({arr: [], name: 'none'});
  ...
  useEffect(() => {
    const first = async () => {
      ...
      const filtered_payload = payload.filter((el) => el.baseRefName === 'main' && !bots.includes(el.author.login));

      // 3) Map over each PR and append their event timeline
      const timelinePromises: any[] = [];
      filtered_payload.forEach((item, ind) => {
        timelinePromises[ind] = fetchPullRequestTimelineItems(item.number).then((timeline) => ({
          ...item,
  ...
  const number_of_authors_who_merged = `Number of PR Authors Who Merged: ${monthAtAGlance.countUniqueAuthors(data)}`
  const number_of_reviewers = `Number of Reviewers: ${monthAtAGlance.countUniqueReviewers(data)}`
  const number_of_comments = `Number of Comments: ${sum(numberOfCommentsArray)}`
  ...
  return (
    <div className="App">
      ...
      <div className="prettyData">
        ...
        <div><h2>{number_of_prs_merged}</h2></div>
        <div><h2>{number_of_authors_who_merged}</h2></div>
        <div><h2>{number_of_reviewers}</h2></div>
        <div><h2>{number_of_comments}</h2></div>
        <div><h2>{lines_added}</h2></div>
        <div><h2>{lines_deleted}</h2></div>

        <h1>Metrics</h1>
        <MetricsHeadline onClick={setActiveArray} arr={timeToMergeArray} name={'Time to Merge'} isTimeUnit />
        <MetricsHeadline onClick={setActiveArray} arr={timeToFirstCommentArray} name={'Time to First Comment or Approval'} isTimeUnit />
        <MetricsHeadline onClick={setActiveArray} arr={timeInApprovedStateArray} name={'Time in Approved State'} isTimeUnit />
        <MetricsHeadline onClick={setActiveArray} arr={numberOfCommentsArray} name={'Number of Comments'} isTimeUnit={false} />
        <MetricsHeadline onClick={setActiveArray} arr={numberOfLinesAddedArray} name={'Number of Lines Added'} isTimeUnit={false} />
        <MetricsHeadline onClick={setActiveArray} arr={numberOfLinesDeletedArray} name={'Number of Lines Deleted'} isTimeUnit={false} />

        <h3>{`Now showing: ${activeArray.name}`}</h3>
        <table>
          <tbody>
            <tr>
              <td><RawDataTextArea arr={activeArray.arr} /></td>
              <td><GroupIntervalButton arr={activeArray.arr} /> </td>
            </tr>
          </tbody>
        </table>
